...
class TestDaemonMainStartSafety(unittest.TestCase):
    def test_start_reuses_daemon_when_ipc_recovers_during_grace_period(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch.object(daemon_main, "default_paths", return_value=paths), patch.object(
                daemon_main, "call_daemon", side_effect=[{"ok": False}, {"ok": False}, {"ok": True}]
    ...
    def test_start_spawns_when_live_pid_exits_during_grace_period(self) -> None:
        from cccc.daemon.server import DaemonPaths
        from cccc import daemon_main
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            out = io.StringIO()
            with patch.object(daemon_main, "default_paths", return_value=paths), patch.object(
                daemon_main, "call_daemon", side_effect=[{"ok": False}, {"ok": False}, {"ok": False}]
            ), patch.object(daemon_main, "read_pid", return_value=12345), patch.object(
