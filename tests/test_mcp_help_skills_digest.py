...
class TestMcpHelpSkillsDigest(unittest.TestCase):
    def test_cccc_help_appends_runtime_skill_digest(self) -> None:
        ...
        self.assertIn("## Ralph Workflow", markdown)
        ...
        self.assertIn('`cccc_model(action="list")`', markdown)
        self.assertIn("## Role Notes", markdown)
        self.assertIn("## Foreman", markdown)
        self.assertIn("## Peer", markdown)
        self.assertIn("Do not execute implementation tasks yourself", markdown)
        self.assertIn("Do not spawn extra workers or re-plan the workflow unless foreman asks.", markdown)
